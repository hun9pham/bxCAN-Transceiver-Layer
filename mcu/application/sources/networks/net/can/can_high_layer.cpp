...
/* Extern variables ----------------------------------------------------------*/
uint8_t (*pf_txCanCb)(uint32_t destNodeId, uint8_t *data, uint16_t len);
...
static CanRecvParserState_t rxFrameRecvParserState = PARSER_STATE_SOF;
...
static uint8_t sendFrameBlock(CanFrame_t *frameTrans) {
    ...
    /* Send frame header */
    error += pf_txCanCb(frameTrans->header.destNodeId, (uint8_t*)&frameTrans->header, sizeof(frameHeader_t));

    /* Send frame data */
    error += pf_txCanCb(frameTrans->header.destNodeId, frameTrans->payload, frameTrans->header.len);

    /* Send frame footer */
    error += pf_txCanCb(frameTrans->header.destNodeId, (uint8_t*)&frameTrans->footer, sizeof(frameFooter_t));

    if (CAN_ErrorCode_NoErr == error) {
        return CANBUS_RET_NOERR;
    }
    CAN_DBG(TAG, "ERROR: 0x%X", error);
    
    return CANBUS_RET_ERR;
}

/* Function implementation ---------------------------------------------------*/
void TaskCanHighLayer(ak_msg_t *msg) {
    switch (msg->sig) {
    case SL_CAN_HIGH_PACKED_FRAME_HEADER: {
        CAN_DBG_SIG(TAG, "SL_CAN_HIGH_PACKED_FRAME_HEADER");
        ...
        */
        ...
        if (CANTX_STATE_IDLE == CanTransmitterState) {
            ...
            txCanPhy.header.srcNodeId = CANBUS_ID_OPTION;
            ...
            txPduIdAtv = pduId;
            txFailedRetryCnt = 0;
            task_post_pure_msg(SL_TASK_CAN_HIGH_LAYER_ID, SL_CAN_HIGH_PACKED_FRAME_DATA);
        }
        else {
    ...
    case SL_CAN_HIGH_PACKED_FRAME_DATA: {
        CAN_DBG_SIG(TAG, "SL_CAN_HIGH_PACKED_FRAME_DATA");
        ...
        */
        ...
        task_post_pure_msg(SL_TASK_CAN_HIGH_LAYER_ID, SL_CAN_HIGH_SEND_FRAME);
    }
    break;

    case SL_CAN_HIGH_SEND_FRAME: {
        CAN_DBG_SIG(TAG, "SL_CAN_HIGH_SEND_FRAME");

        if (CANTX_STATE_SENDING == CanTransmitterState) {
            sendFrameBlock(&txCanPhy);
            timer_set(SL_TASK_CAN_HIGH_LAYER_ID, SL_CAN_HIGH_RECV_ACK_TIMEOUT, CANBUS_FRAME_RECEIVE_TIMEOUT_AFTER, TIMER_ONE_SHOT);   
        }
    }
    break;

    case SL_CAN_HIGH_RECV_FRAME: {
        CAN_DBG_SIG(TAG, "SL_CAN_HIGH_RECV_FRAME");
        ...
        */
        ...
        case FRAME_TYPE_REQ_ACK_FROM_RECEIVER: {
            CAN_DBG(TAG, "FRAME_TYPE_REQ_ACK_FROM_RECEIVER");
            ...
            timer_set(SL_TASK_CAN_HIGH_LAYER_ID, SL_CAN_HIGH_RECV_FRAME_TIMEOUT, CANBUS_FRAME_RECEIVE_TIMEOUT_AFTER, TIMER_ONE_SHOT);
            /* Respond ACK */
            task_post_common_msg(SL_TASK_CAN_HIGH_LAYER_ID, SL_CAN_HIGH_SEND_ACK, (uint8_t*)&ACK_RespTo, sizeof(uint32_t));
        }
        break;

        case FRAME_TYPE_RECEIVER_RESP_ACK: {
            CAN_DBG(TAG, "FRAME_TYPE_RECEIVER_RESP_ACK");
            uint32_t ACK_RecvFrom = frameRecv->header.srcNodeId;
            task_post_common_msg(SL_TASK_CAN_HIGH_LAYER_ID, SL_CAN_HIGH_RECV_ACK, (uint8_t*)&ACK_RecvFrom, sizeof(uint32_t));
        }
        break;

        case FRAME_TYPE_DATA_TRANSMIT_COMPLETED: {
            CAN_DBG(TAG, "FRAME_TYPE_DATA_TRANSMIT_COMPLETED");
            CanReceiverState = CANRX_STATE_IDLE;
            timer_remove_attr(SL_TASK_CAN_HIGH_LAYER_ID, SL_CAN_HIGH_RECV_FRAME_TIMEOUT);
            task_post_common_msg(SL_TASK_CAN_LOW_LAYER_ID, SL_CAN_LOW_PARSER_FRAME_DATA_RECV, &rxPduIdAtv, sizeof(uint8_t));
        }
        break;
    ...
    case SL_CAN_HIGH_SEND_ACK: {
        CAN_DBG_SIG(TAG, "SL_CAN_HIGH_SEND_ACK");
        ...
        frameAckSend.header.srcNodeId = CANBUS_ID_OPTION;
    ...
    case SL_CAN_HIGH_RECV_ACK: {
        CAN_DBG_SIG(TAG, "SL_CAN_HIGH_RECV_ACK");

        uint32_t ackFromNodeId;
        memcpy(&ackFromNodeId, get_data_common_msg(msg), sizeof(uint32_t));
        CAN_DBG(TAG, "0x%X vs 0x%X", ackFromNodeId, txCanPhy.header.destNodeId);
        if (ackFromNodeId == txCanPhy.header.destNodeId) {
            txFrameSize -= txCanPhy.header.len;
            timer_remove_attr(SL_TASK_CAN_HIGH_LAYER_ID, SL_CAN_HIGH_RECV_ACK_TIMEOUT);
            if (txFrameSize > 0) {
                task_post_pure_msg(SL_TASK_CAN_HIGH_LAYER_ID, SL_CAN_HIGH_PACKED_FRAME_DATA);
            }
            else {
                task_post_common_msg(SL_TASK_CAN_HIGH_LAYER_ID, SL_CAN_HIGH_SEND_FRAME_COMPLETED, (uint8_t*)&ackFromNodeId, sizeof(uint32_t));
            }
        }
        else {
            /*
             ...
             -
            */
            CAN_DBG(TAG, "ERROR: ACK Respond lost address !!!");
        }
    }
    break;

    case SL_CAN_HIGH_SEND_FRAME_COMPLETED: {
        CAN_DBG_SIG(TAG, "SL_CAN_HIGH_SEND_FRAME_COMPLETED");
        ...
        ackComplete.header.srcNodeId = CANBUS_ID_OPTION;
    ...
    case SL_CAN_HIGH_RECV_FRAME_TIMEOUT: {
        CAN_DBG_SIG(TAG, "SL_CAN_HIGH_RECV_FRAME_TIMEOUT");

        CanReceiverState = CANRX_STATE_IDLE;
        rxFrameRecvParserState = PARSER_STATE_SOF;
        CanPduManager.freePdu(rxPduIdAtv);
    }
    break;

    case SL_CAN_HIGH_RECV_ACK_TIMEOUT: {
        CAN_DBG_SIG(TAG, "SL_CAN_HIGH_RECV_ACK_TIMEOUT");
...
void CanParserByteRecv(uint8_t c) {
	...
	case PARSER_STATE_TYPE: {
		...
		rxFrameRecvParserState = PARSER_STATE_LEN;
		dataIndxRecv = 0;
	}
	break;
	...
	case PARSER_STATE_LEN: {
		...
		if (dataIndxRecv == sizeof(rxCanPhy.header.len)) {
            if (rxCanPhy.header.len > CAN_PHY_FRAME_DATA_SIZE) {
                ...
                rxFrameRecvParserState = PARSER_STATE_FCS;
            }
            else {
                dataIndxRecv = 0;
                rxFrameRecvParserState = PARSER_STATE_DATA;
            }
		}
	}
	break;
    ...
    case PARSER_STATE_FCS: {
        ...
        if (fCsCalc == rxCanPhy.footer.fcs) {
            rxFrameRecvParserState = PARSER_STATE_EOF;
        }
        else {
            rxFrameRecvParserState = PARSER_STATE_SOF;
        }
	...
	case PARSER_STATE_EOF: {
		if (CAN_PHY_EOF == c) {
            ...
            task_post_common_msg(SL_TASK_CAN_HIGH_LAYER_ID, SL_CAN_HIGH_RECV_FRAME, (uint8_t*)&rxCanPhy, sizeof(CanFrame_t));
		}
		rxFrameRecvParserState = PARSER_STATE_SOF;
	}
	break;
